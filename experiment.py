...
def f(filename, n_source, n_target, prop_target, prop_source = 0.5,\
     labeled = True, d = 4, distance = 4, kernel_df = 3, beta = 3, iteration = 0):
    
    D = data.DataGenerator(d = d)
    x_source, y_source = D.getData(n_source, prop_source)
    x_target, y_target = D.getData(n_target, prop_target)
    x_test, y_test = D.getData(100, prop_target)
...
if __name__ == "__main__":
    ...
    f('exp1', int(n_source), int(n_target), 0.75, labeled=labeled, kernel_df=int(kernel_df), beta= beta, iteration=int(iteration))
